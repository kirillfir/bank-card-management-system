...
CREATE TABLE roles (
    ...
    name VARCHAR(50) NOT NULL UNIQUE -- ROLE_USER, ROLE_ADMINь
);
...
-- Устанавливаем временное значение, если в базе уже есть люди, иначе UNIQUE не применится
UPDATE users SET email = username || '@mail.com' WHERE email IS NULL;
